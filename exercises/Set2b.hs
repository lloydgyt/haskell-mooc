...
countdown t = "Ready! " ++ timeSequence t ++ "Liftoff!"
              where 
                timeSequence 1 = "1... " -- arm-length recursion!
                timeSequence t = show t ++ "... " ++ timeSequence (t - 1)
...
smallestDivisor n = helper 2
                    ...
                    -- where helper x = if (mod n x) == 0 then x else helper (x + 1)
-- check from smaller number
